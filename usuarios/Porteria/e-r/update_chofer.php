...
try {
    ...
    if (!isset($_POST['id'])) {
        sendResponse(false, 'ID no proporcionado');
    }
    ...
    $sql = "UPDATE data_chofer SET " . implode(", ", array_map(function($field) {
        return "$field = ?";
    ...
    error_log("Error en update_chofer.php: " . $e->getMessage());
    sendResponse(false, 'Error al actualizar el chofer: ' . $e->getMessage());
} 